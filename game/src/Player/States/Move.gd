...
export var gravity: int = -9.8
export var jump_impulse = 25
export (float, 0.1, 20.0, 0.1) var rotation_speed_factor: = 5.0
...
func physics_process(delta: float) -> void:
	...
	velocity = player.move_and_slide(velocity, Vector3.UP, false, 4, PI/4, false)
...
func calculate_velocity(
		...
		velocity_new.y = velocity_current.y + (gravity * mass) * delta
